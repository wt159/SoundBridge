...
static constexpr size_t kMaxMetadataSize = 3 * 1024 * 1024;

struct MemorySource : public DataSourceBase, public NonCopyable {
...
bool ID3::parseV2(DataSourceBase *source, off64_t offset)
{
    ...
    id3_header header;
    if (source->readAt(offset, &header, sizeof(header)) != (ssize_t)sizeof(header)) {
        return false;
    }
    ...
    if (size > kMaxMetadataSize) {
        LOG_ERROR(LOG_TAG, "skipping huge ID3 metadata of size %zu", size);
        return false;
    }
    ...
    if (header.version_major == 4) {
        ...
        if (copy == NULL) {
            free(mData);
            mData = NULL;
            LOG_ERROR(LOG_TAG, "b/24623447, no more memory");
            return false;
        }
        ...
        if (!success) {
            ...
            if (success) {
                LOG_DEBUG(LOG_TAG, "Had to apply the iTunes hack to parse this ID3 tag");
            }
        }
        ...
        LOG_DEBUG(LOG_TAG, "removing unsynchronization");
    ...
    if (header.version_major == 3 && (header.flags & 0x40)) {
        ...
        if (extendedHeaderSize > SIZE_MAX - 4) {
            free(mData);
            mData = NULL;
            LOG_ERROR(LOG_TAG, "b/24623447, extendedHeaderSize is too large");
            return false;
        }
        ...
        if (extendedHeaderSize >= 6) {
            ...
            if (extendedHeaderSize >= 10) {
                ...
                if (paddingSize > SIZE_MAX - mFirstFrameOffset) {
                    LOG_ERROR(LOG_TAG, "b/24623447, paddingSize is too large");
                }
    ...
    while (mSize >= 10 && offset <= mSize - 10) {
        ...
        if ((flags & 2) && (dataSize >= 2)) {
            ...
            if (readOffset <= oldSize) {
                ...
                LOG_ERROR(LOG_TAG, "b/34618607 (%zu %zu %zu %zu)", readOffset, writeOffset, oldSize,
                          mSize);
            }
        }
        flags &= ~2;
...
void ID3::Iterator::getID(std::string *id) const
{
    id->append("");
    ...
    if (mParent.mVersion == ID3_V2_2) {
        id->append((const char *)&mParent.mData[mOffset], 3);
    } else if (mParent.mVersion == ID3_V2_3 || mParent.mVersion == ID3_V2_4) {
        id->append((const char *)&mParent.mData[mOffset], 4);
    } else {
...
void ID3::Iterator::getString(std::string *id, std::string *comment) const
{
    getstring(id, false);
    if (comment != NULL) {
...
// "otherdata" to true.
void ID3::Iterator::getstring(std::string *id, bool otherdata) const
{
    id->append("");
    ...
    if (mParent.mVersion == ID3_V1 || mParent.mVersion == ID3_V1_1) {
        ...
        id->append((const char *)frameData, mFrameSize);
        return;
    }
    ...
    if (encoding == 0x00) {
        ...
        id->append((const char *)frameData + 1, n);
    } else if (encoding == 0x03) {
        // supposedly UTF-8
        id->append((const char *)(frameData + 1), n);
    } else if (encoding == 0x02) {
        LOGW("ID3 2.2 tags with UCS-2 encoding are not supported");
        // not supported
...
void ID3::Iterator::findFrame()
{
    for (;;) {
        ...
        if (mParent.mVersion == ID3_V2_2) {
            ...
            mFrameSize = (mParent.mData[mOffset + 3] << 16) | (mParent.mData[mOffset + 4] << 8)
            ...
            if (mOffset + mFrameSize > mParent.mSize) {
                LOG_DEBUG(LOG_TAG,
                          "partial frame at offset %zu (size = %zu, bytes-remaining = %zu)",
            ...
            if (mParent.mVersion == ID3_V2_4) {
                if (!ParseSyncsafeInteger(&mParent.mData[mOffset + 4], &baseSize)) {
                    return;
                }
            } else {
            ...
            if ((mParent.mVersion == ID3_V2_4 && (flags & 0x000c))
                ...
                LOG_DEBUG(LOG_TAG,
                          "Skipping unsupported frame (compression, encryption "
...
const void *ID3::getAlbumArt(size_t *length, std::string *mime) const
{
    *length = 0;
    // mime->append("");
    ...
    while (!it.done()) {
        ...
        if (mVersion == ID3_V2_3 || mVersion == ID3_V2_4) {
            ...
            if (mimeLen > size - consumed) {
                LOG_WARNING(LOG_TAG, "bogus album art size: mime");
                return NULL;
            }
            // mime->append((const char *)&data[consumed]);
            consumed += mimeLen;
            ...
            consumed++;
            if (consumed >= size) {
                LOG_WARNING(LOG_TAG, "bogus album art size: pic type");
                return NULL;
            }
            ...
            if (consumed >= size) {
                LOG_WARNING(LOG_TAG, "bogus album art size: description");
                return NULL;
            }
...
bool ID3::parseV1(DataSourceBase *source)
{
    ...
    if (source->readAt(size - V1_TAG_SIZE, mData, V1_TAG_SIZE) != (ssize_t)V1_TAG_SIZE) {
        free(mData);
        mData = NULL;

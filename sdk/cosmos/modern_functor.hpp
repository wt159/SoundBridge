...
#define make_globle_curry_functor(NAME, F) \
    define_functor_type(F);                \
    const auto NAME = fn_to_curry_functor(tfn_##F());
...
public:
    fn_chain()
        : functions_(std::tuple<>())
    {
    }
    fn_chain(std::tuple<FNs...> functions)
...
template <>
int set_empty<int>() { return 0; }
template <>
string set_empty<string>() { return ""; }
...
public:
    ...
    maybe(T&& x)
        : x(std::forward<T>(x))
        , state(maybe_state::normal)
    {
    }
    maybe()
        : x(set_empty<T>())
        , state(maybe_state::empty)
    {
    }
...
public:
    ...
    const string name;
    const int parent;
    UserData(int id, string name, int parent)
        : id(id)
        , name(name)
        , parent(parent)
    {
